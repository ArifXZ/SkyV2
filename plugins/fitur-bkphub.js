...
try {
let f = await fetch(`https://api.lolhuman.xyz/api/xnxxsearch?apikey=42092e951066bf09423f6ade&query=${text}`)
let xx = await f.json()
let str = xx.result.map((v, index) => {
        return `${1 + index}. Judul *${v.title}*
views: ${v.views}
...
link: ${v.link}`.trim()
    ...
    try {
  let json = await fetch(`https://api.lolhuman.xyz/api/xnxx?apikey=42092e951066bf09423f6ade&url=${text}`)
